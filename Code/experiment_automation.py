...
# Splitting for hardware setup with two GPUs
splits_size_half = splits_size // 2
if args.half == 0:
    split_nrs = range(splits_size_half)
    split_generator = itertools.islice(split_generator, 0, splits_size_half, 1)
    GPU_DEVICE = 0

else:
    split_nrs = range(splits_size_half, splits_size)
    split_generator = itertools.islice(split_generator, splits_size_half, splits_size, 1)
    GPU_DEVICE = 1

# When split selected, further reduce split_generator
...
for split_nr, (train_indices, test_indices) in zip(split_nrs, split_generator):
    ...
    with open('Streams/{}_{}_{}_{}_stdout.txt'.format(args.model_name, args.type, CUR_DOMAIN_FACTOR_NAME, str(split_nr)), 'w') as stdoutFile, \
             open('Streams/{}_{}_{}_{}_stderr.txt'.format(args.model_name, args.type, CUR_DOMAIN_FACTOR_NAME, str(split_nr)), 'w') as stderrFile:
