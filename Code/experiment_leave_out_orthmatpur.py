...
mpursuit_model = OrthMatchPursuit(n_rx=n_rx, n_adc=n_adc, rmin=0., rmax=5., theta_min=-70., theta_max=70., rsamp=100,
                                 theta_samp=100, slow_time_seq_len=args.observation_size, backbone_name=args.model_name + "_backbone",
                                 num_classes=class_nr, b_size=args.batch_size).get_model()
...
train_history = train_model.fit(x=train_set, epochs=args.epoch_size, steps_per_epoch=(len(train_instance_indices) * step_multiplier) // args.batch_size,
...
metrics_frame = pd.DataFrame(data=[[acc, precision, recall, f_score]], columns=['A', 'P', 'R', 'F'])

if not os.path.exists(os.path.abspath('results/leave-out/')):
    os.mkdir(path=os.path.abspath('results/leave-out/'))

results_frame = pd.concat([history_frame, metrics_frame], axis=1)
results_frame.to_csv("results/leave-out/{}_results_dfn_{}_ps_{}_ol_{}_cvs_{}_{}.csv".format(args.model_name, args.domain_factor_name, str(args.prediction_size), str(int(args.overlap * 100.0)), str(args.crossval_split), datetime.datetime.now().strftime('%Y-%m-%d_%H-%M')))
